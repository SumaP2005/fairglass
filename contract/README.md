...
2. Run the local Midnight proof server (`midnight-proof-server` or whatever
   the starter kit provides) — keep it running all weekend.
...
3. Emit a `FairnessReceipt` event containing: policy hash, decision, timestamp
   — and nothing else (no candidate data).
4. Reject proofs that used a forbidden attribute (this is the biased-model
   demo moment).

## Files (to be added during the hackathon)

- `policy.compact` — the actual Compact contract source
- `deploy.md` — notes on the testnet deployment (contract address, etc.)
